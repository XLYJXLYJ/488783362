...
module.exports = {
    res: {
        ...
        verify(req, res, next) {
            ...
            if (userid) {
                let codeSql = 'SELECT id,username,headportrait,message FROM manykit_user WHERE id = ?';
        ...
        signup(req, res, next) {
            ...
            publicSql(codeSql, codeParams).then((result) => {
                ...
                if (result.length !== 0) {
                    ...
                    //验证用户名是否存在
                    let codeSql = 'SELECT username FROM manykit_user WHERE username = ?';
                    let codeParams = [username];
                    publicSql(codeSql, codeParams).then((result) => {
                        let data = JSON.parse(JSON.stringify(result));
                        if (result.length === 0) {
                            let codeSql = "INSERT INTO `manykit_user` SET `username`=?, `mail`=?, `password`=?,`cdkey` = ?,`state` = ?";
                            let codeParams = [username, mail, password, cdkey, 0];
                            publicSql(codeSql, codeParams).then((result) => {
                                let data = JSON.parse(JSON.stringify(result));
                                let resDate = {
                                    msg: '请前往邮箱' + mail + ',激活账号',
        ...
        active(req, res, next) {
            ...
            publicSql(codeSql, codeParams).then((result) => {
                if (result.length !== 0) {
                    ...
                    // 用户不存在
                    res.set('refresh', '5;url=http://www.manykit.com/' + website + '');
        ...
        login(req, res, next) {
            let username = validator.trim(req.body.username).toLowerCase();
            let passwordRsa = validator.trim(req.body.password);
            let password;
                ...
                if (result.length !== 0) {
                    ...
                    if (data.state == '0') {
                        ...
                        if (password == resPassword) {
                            ...
                            syncUser.mongo.getID(username, (id) => {
                                if (id) {
        ...
        thirdlogin(req, res, next) {
            ...
            //         let codeSql = "INSERT INTO `manykit_user` SET `username`=?,`state` = ?,`type` = ?,`openid` = ?";
            //         let codeParams = [username, 1, type, openid];
            //         publicSql(codeSql, codeParams).then((result) => {
            //             let codeSql = 'SELECT id FROM `manykit_user` WHERE `openid` = ? AND `type` = ?';
            //             let codeParams = [openid, type];
            //             publicSql(codeSql, codeParams).then((result) => {
        ...
        userinfo(req, res, next) {
            ...
            if (state === 1) {
                ...
                codeSql = 'SELECT id,username,age,realname,sex,imgBuffer,aboutme,doing,mycollect,myattention,attentionme,coverworkid FROM manykit_user  WHERE `id` = ? ';
                codeParams = [userid];
            }
            publicSql(codeSql, codeParams).then((result) => {
                let data = JSON.parse(JSON.stringify(result))[0];
                            ...
                            publicSql(codeSql, codeParams).then((result) => {
                                ...
                                for (let k = 0; k < temp_mycollect.length; k++) {
                                    ...
                                    if (parseInt(temp_mycollect[k].surfaceplot) === 9) {
                                        ...
                                        let name = '' + id + '' + '' + imgname + '';
                                        let temp_name = './webs/codeplay/static/headPortraits/' + name;
                                        ...
                                        temp_mycollect[k].imgBuffer = '/codeplay/static/headPortraits/' + name;
                                    } else {
                                        temp_mycollect[k].imgBuffer = '/codeplay/static/publish/surfaceplot' + temp_mycollect[k].surfaceplot + '.png';
                            ...
                            publicSql(codeSql, codeParams).then((result) => {
                                ...
                                for (let i = 0; i < data.length; i++) {
                                    if (data[i].imgBuffer) {
                                        ...
                                        data[i].imgBuffer = '/codeplay/static/headPortraits/' + name;
                                    }
                                }
                                responseResult.success(res, data)
                            }).catch((err) => {
        ...
        upload(req, res, next) {
            ...
            for (let i = 0; i < req.files.length; i++) {
                ...
                if (state === 1) {
                    ...
                    codeSql = "INSERT INTO `manykit_userproject` SET `userid`=?,`name`=?,`title`=?, `desc`=?,`content` = ?,`create_time` = ?";
                    codeParams = [userid, fileOriginalname, title, desc, fileBuffer, create_time];
                } else if (state === 2) {
        ...
        dealfile(req, res, next) {
            ...
            let state = parseInt(req.body.state);
            if (tool.ArrIsEmpty([userid, id, state])) {
                return responseResult.error(res, '信息不完整');
            }
        ...
        getfile(req, res, next) {
            ...
            if (state === 1) {
                ...
                codeSql = 'SELECT id,title,`desc`,praise,praisetotal,collect,collecttotal,looktotal,create_time,publish_time,recycle_time,userid FROM manykit_userproject WHERE `id` = ?';
                codeParams = [id];
            } else {
            ...
            publicSql(codeSql, codeParams).then((result) => {
                if (result.length !== 0) {
                    if (state === 1) {
                        ...
                        let temp_name = './webs/codeplay/file/' + id + '.xml';
        ...
        filelist(req, res, next) {
            ...
            if (typeof(userid) === 'undefined') {
                let start = (pagenum - 1) * pagesize;
                codeSql = 'SELECT id,title,`desc`,create_time,looktotal,praisetotal,collecttotal,imgBuffer,imgname,surfaceplot FROM manykit_userproject  WHERE `state` = ? limit ?, ?';
                codeParams = [state, start, pagesize];
            } else {
                codeSql = 'SELECT username FROM manykit_user WHERE `id` = ?';
                codeParams = [userid];
                publicSql(codeSql, codeParams).then((result) => {
                    if (result.length !== 0) {
                        let data = JSON.parse(JSON.stringify(result));
                ...
                if (typeof(state) === "undefined") {
                    codeSql = 'SELECT id,title,`desc`,create_time,looktotal,praisetotal,collecttotal,state,imgBuffer,imgname,surfaceplot FROM manykit_userproject WHERE `userid` = ? AND (`state` = 0 OR `state` = 1)';
                    codeParams = [userid];
                } else {
                    codeSql = 'SELECT id,title,`desc`,create_time,looktotal,state,update_time,praisetotal,collecttotal,imgBuffer,imgname,surfaceplot FROM manykit_userproject WHERE `userid` = ? AND `state` = ?';
            ...
            if (sortstate === 1) {
                ...
                let start = (pagenum - 1) * pagesize;
                codeSql = 'SELECT id,title,`desc`,create_time,looktotal,praisetotal,collecttotal,imgBuffer,imgname,surfaceplot FROM manykit_userproject  WHERE `state` = ? order by praisetotal desc limit ?, ?';
                ...
                if (result.length !== 0) {
                    ...
                    for (let i = 0; i < data.length; i++) {
                        ...
                        if (data[i].surfaceplot === '9') {
                            ...
                            data[i].imgBuffer = '/codeplay/static/publish/surfaceplot' + data[i].surfaceplot + '.png';
        ...
        resourcelist(req, res, next) {
            ...
            publicSql(codeSql, codeParams).then((result) => {
                if (result.length !== 0) {
                    ...
                    //         name = id + '.' + type;
                    //     }
                    //     let temp_name = './webs/codeplay/static/resource/' + name;
                    //     let writeStream = fs.createWriteStream(temp_name);
